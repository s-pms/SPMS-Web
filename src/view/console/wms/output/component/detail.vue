...
<template>
  <ADialog :loading="isLoading" height="80%" title="出库单详情" width="80%" @on-confirm="onConfirm"
           @on-cancel="onCancel">
    <el-form label-width="120px" @submit.prevent>
      <AGroup :column="3" title="出库单">
        <BillFormCode :bill="formData"/>
        <AFormField disabled field="type"/>
        <el-form-item v-if="OutputTypeEnum.SALE.equalsKey(formData.type)" label="销售单号">
          <el-link @click="AirDialog.show(SaleDetail, formData.sale)">
            {{ formData.sale.billCode }}
          </el-link>
        </el-form-item>
        <el-form-item v-if="OutputTypeEnum.MOVE.equalsKey(formData.type)" label="移库单号">
          <el-link @click="AirDialog.show(MoveDetail, formData.move)">
            {{ formData.move.billCode }}
          </el-link>
        </el-form-item>
        <el-form-item v-if="OutputTypeEnum.PICKING.equalsKey(formData.type)" label="领料单号">
      ...
      <AGroup title="出库明细">
        <ATable :data-list="formData.details" :entity="OutputDetailEntity" :field-list="OutputDetailEntity.getTableFieldConfigList().filter(
          ...
          <template #endRow="{ data }">
            <AButton :disabled="OutputStatusEnum.OUTPUTTING.notEqualsKey(formData.status)
              " icon-button tooltip="添加完成" type="CHECKIN" @click="onAddFinish(data, formData.id)"/>
          </template>
        </ATable>
      </AGroup>
    </el-form>
