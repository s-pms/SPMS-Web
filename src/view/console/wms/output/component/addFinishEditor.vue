...
<template>
  <ADialog :disable-confirm="formData.quantity <= 0" :form-ref="formRef" :loading="isLoading" title="确认出库"
           width="600px"
           @on-confirm="onSubmit" @on-cancel="onCancel">
    <el-form ref="formRef" :model="formData" :rules="OutputDetailService.createValidator(formData)" label-width="120px"
             @submit.prevent>
      <el-form-item label="物料信息">
        <el-input v-model="material.name" disabled/>
      </el-form-item>
      <el-form-item label="库存来源" prop="inventoryId">
        <AInput v-model="formData.storageName" :entity="OutputDetailEntity" placeholder="请选择库存来源..."
                @click="selectInventory"/>
      </el-form-item>
      <el-form-item v-if="formData.inventory" label="当前库存">
        <el-input v-model="formData.inventory.quantity" disabled>
          <template #append>
            {{ material.unit.name }}
          </template>
        </el-input>
      ...
      <el-form-item label="待出数量">
        <el-input v-model="quantity" disabled>
          <template #append>
            {{ material.unit.name }}
          </template>
        </el-input>
